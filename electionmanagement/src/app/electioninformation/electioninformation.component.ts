...
export class ElectioninformationComponent implements OnInit {
  ...
  View(electioninformation) {
    const url =
      "view/demo/" + electioninformation.data.election_ID + "/DemoviewOne";
  ...
  Delete(row) {
    this.electioninformationservice.delete(row.data.election_ID).subscribe(
      (response) => {
        if (response) {
          if (response.error && response.status) {
            ...
            this.toastrservice.info("Election Information Deleted");
  ...
  add(electioninformation) {
    ...
    this.electioninformationservice.add(electioninformation).subscribe(
      (response) => {
        if (response) {
          console.log(response);
          ...
          if (response.error && response.status) {
            ...
            this.toastrservice.success("Success");
            this.toastrservice.info("New Election Record Added");
            this.electioninformation = response;
            this.getAll();
  ...
  update(electioninformation) {
    ...
    electioninformation.election_TYPE = 2;
    this.electioninformationservice
      .update(electioninformation, electioninformation.election_ID)
      .subscribe(
        (response) => {
          if (response) {
            if (response.error && response.status) {
              ...
              this.toastrservice.success("Success");
              this.toastrservice.info("Election Information Updated");
              this.electioninformation = response;
              this.getAll();
              $("#editModal").modal("hide");
            } else {
  ...
  getElectionType() {
    this.electiontypeActive = ["GENERAL"];
  }
}
